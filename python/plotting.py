...
def plot_contour(original, decompressed, title=""):
    ...
    magnitude_o = np.sqrt(original[0, 0, 0,:,:]** 2 + original[0, 1, 0,:,:]**2)
    print(magnitude_o.shape)
    magnitude_r = np.sqrt(decompressed[0,0,0,:,:]**2+decompressed[0,1,0,:,:]**2)
    error = original - decompressed
    n = original.shape[-1]
    m = original.shape[-2]
    x = np.arange(n)
    ...
    strm = plt.streamplot(x, y, u=original[0,0, 0,:,:].reshape(n, m), v=original[0,1, 0,:,:].reshape(n, m),color=magnitude_o, linewidth=2, cmap='autumn')
    plt.xlabel('original')
    plt.subplot(1,3,2)
    plt.streamplot(x, y, u=decompressed[0,0, 0,:,:].reshape(n, m), v=decompressed[0,1, 0,:,:].reshape(n, m),color=magnitude_r, cmap='autumn')
    plt.xlabel('decompressed')
    plt.subplot(1,3,3)
