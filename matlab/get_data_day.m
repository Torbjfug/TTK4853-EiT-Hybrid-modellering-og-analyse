function get_data_day(year, month, day,folder)
    ...
    x_begin = 2; x_end = 133; y_begin = 2; y_end = 133; l_begin = 9;l_end =40;
    ...
    %geopotential_height_ml =  get_data(year, month, day, 'geopotential_height_ml', x_begin, x_end, y_begin, y_end, l_begin, l_end, 0, 12);
    ...
    if any(isnan(x_wind_ml(:))) || any(isnan(y_wind_ml(:))) || any(isnan(upward_air_velocity_ml(:))) || std(x_wind_ml(:))==0 || std(y_wind_ml(:))==0 || std(upward_air_velocity_ml(:))==0 || any(isnan(air_pressure_ml(:))) || std(air_pressure_ml(:))==0
        error('Invalid data') 
    else
